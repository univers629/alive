...
class AliveException(Exception):
    '''
    Alive application error.
    '''
...
def get_path(path: str, create_dirs: bool = True, is_dir: bool = False) -> str:
    ...
    :return: 绝对路径
    '''
    if current_dir().startswith('/var/task') and path.startswith('data/'):
        # Vercel's application directory is read-only; keep runtime data in /tmp.
        full_path = str(Path('/tmp/alive').joinpath(path))
        if create_dirs:
            if is_dir:
